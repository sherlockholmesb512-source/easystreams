...
const SITE_BASE = 'https://cineblog001.cloud';
const SITE_LABEL = 'CineBlog01';

async function searchCandidates(title) {
  const html = await fetchText(`${SITE_BASE}/index.php?do=search&subaction=search&story=${encodeURIComponent(title)}`, {
    Referer: `${SITE_BASE}/`,
