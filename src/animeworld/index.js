...
async function fetchSeriesTitlesFromTmdb(tmdbId) {
  ...
  try {
    ...
    return uniqueStrings(
      [payload && payload.name, payload && payload.original_name]
...
function slugTokens(value) {
  return String(value || "")
    .toLowerCase()
    .split(/[^a-z0-9]+/i)
    .filter((token) => token && !["ita", "sub", "dub"].includes(token));
}
...
async function searchAnimeworldPaths(keyword) {
  const query = String(keyword || "").trim();
  if (!query) return [];
  try {
    const html = await fetchResource(`${getWorldBaseUrl()}/search?keyword=${encodeURIComponent(query)}`, {
...
async function findAnimeworldFallbackPaths(lookup, providerContext, mappingPayload) {
  const contextTmdbId = /^\d+$/.test(String(providerContext?.tmdbId || "").trim())
    ? String(providerContext.tmdbId).trim()
    : null;
  const payloadTmdbId = extractTmdbIdFromMappingPayload(mappingPayload);
  const tmdbId = contextTmdbId || payloadTmdbId;

  const titles = await fetchSeriesTitlesFromTmdb(tmdbId);
  if (titles.length === 0) return [];

  const candidates = new Set();
  for (const title of titles) {
    for (const path of await searchAnimeworldPaths(title)) {
      candidates.add(path);
    }
  }
...
async function getStreams(id, type, season, episode, providerContext = null) {
  try {
    ...
    const extractFromPaths = async (paths, episodeOverride = null) => {
      const perPathStreams = await mapLimit(paths, 3, (path) =>
        extractStreamsFromAnimePath(path, episodeOverride || requestedEpisode, mediaType)
      );
    ...
    if (longSeriesNeedsAbsoluteFallback) {
      const absoluteStreams = await extractFromPaths(animePaths, absoluteEpisode);
